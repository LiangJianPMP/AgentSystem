...
<script type="text/javascript">
        ...
        table.on('tool(user)',function(obj) {
            ...
            if (obj.event === 'del') {
                layer.confirm('真的删除行么', function(index) {
                    var a=data.id;
                    window.location.href = "youhuidel?id="+data.id+"";
                    layer.close(index);
                ...
                $(function () {
                    ...
                    $.ajax({
                        ...
                        success: function (result) {
                            ...
                            var aaa = result[0].id
                            var peizi = result[0].configTypeValue
                            var cnm = result[0].configValue



                            layer.confirm('<div style="padding: 50px 30px;">' + '<b>您正在进行修改操作</b><hr><br><form action="youhuiupdate" method="get" id="Sub">类型名称:<input type="text" name="configTypeName" id="name"><br><br>配置数值:<input type="text" name="configTypeValue" id="pz"><br><br>实际数值:<input type="text" name="configValue" id="sj"><br><br>是否启用:<select  name="isStart" id="index"><option  value="1">启用</option><option   value="2">不启用</option></select><br> <br><input type="hidden" id="sa" name="id"></form>' + '</div>', {
                                btn: ['修改', '取消'] //可以无限个按钮
    ...
    function test(){
        layer.confirm('<div style="padding: 50px 30px;">' + '<b>您正在进行添加操作</b><hr><br><form action="youhuiadd" method="get" id="Sub">类型名称:<input type="text" name="configTypeName" id="name"><br><br>配置数值:<input type="text" name="configTypeValue" id="pz"><br><br>实际数值:<input type="text" name="configValue" id="sj"><br><br>是否启用:<select  name="isStart" id="index"><option  value="1">启用</option><option   value="2">不启用</option></select><br> <br><input type="hidden" value="7" name="configType"></form>' + '</div>', {
            btn: ['添加', '取消'] //可以无限个按钮
            ,yes: function(index, layero){
                $("#Sub").submit();
            }
