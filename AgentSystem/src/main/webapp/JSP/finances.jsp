...
<script type="text/javascript">
        ...
        table.on('tool(user)',function(obj) {
            ...
            if (obj.event === 'del') {
                layer.confirm('真的删除行么', function(index) {
                    var a=data.id;
                    window.location.href = "typedelete?id="+data.id+"";
                    layer.close(index);
                ...
                $(function() {
                    ...
                    $.ajax({
                        ...
                        success : function(result){
                            ...
                            layer.confirm('<div style="padding: 50px 30px;">'+'<b>您正在进行修改操作</b><hr><br><form action="typeupdate" method="post" id="Sub">类型名称:<input type="text" name="type" id="name"><br><br>是否启用:<select  name="enabled" id="index"><option  value="1">启用</option><option   value="2">不启用</option></select><br> <br><input type="hidden" id="sa" name="id"></form>'+'</div>',{
                                btn: ['修改', '取消'] //可以无限个按钮
    ...
    function test(){
        layer.confirm('<div style="padding: 50px 30px;">'+'<b>您正在进行添加操作</b><hr><br><form action="typeadd" method="post" id="Sub">类型名称:<input type="text" name="type" id="name" ><br><br>是否启用:<select  name="enabled"><option selected value="启用">启用</option><option  value="不启用">不启用</option></select><br> <br></form>'+'</div>',{
            btn: ['添加', '取消'] //可以无限个按钮
            ,yes: function(index, layero){
                $("#Sub").submit();
            }
